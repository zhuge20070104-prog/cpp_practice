...
struct Query {
    ...
    Query(std::string const& creature_name_, Argument argument_, int result_):
...
struct Creature {
    ...
    Creature(Game& game_, int attack_, int defense_, std::string const& name_):
...
public:
    CreatureModifier(Game& game_, Creature& creature_): 
        game(game_), creature(creature_) {}
...
public:
    DoubleAttackModifier(Game& game_, Creature& creature_):
        CreatureModifier(game_, creature_) {
...
class IncreaseDefenseModifier: public CreatureModifier {
    ...
    int attack {0};
public:
    IncreaseDefenseModifier(Game& game_, Creature& creature_):
        CreatureModifier(game_, creature_) {
