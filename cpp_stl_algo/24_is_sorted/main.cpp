...
TEST_CASE("Is sorted") {
    ...
    std::cout << "Common is sorted: " << is_sorted << std::endl;

    bool is_sorted_abs = std::is_sorted(std::begin(v), std::end(v), [](int elem1, int elem2) {
        return abs(elem1) > abs(elem2);
    });
    std::cout << "Absolute is sorted: " << is_sorted_abs << std::endl;
}