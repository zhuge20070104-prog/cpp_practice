...
void test_sum_array_with_timing(int block_size) {
    ...
    gpuErrCheck(cudaMemcpy(d_c, h_c, NO_BYTES, cudaMemcpyHostToDevice));
    htod_end = clock();
    ...
    dtoh_start = clock();
    gpuErrCheck(cudaMemcpy(gpu_results, d_d, NO_BYTES, cudaMemcpyDeviceToHost));
    dtoh_end = clock();
    ...
    compare_results(h_d, gpu_results, size);
