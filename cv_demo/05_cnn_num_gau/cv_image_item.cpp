...
QString CVImageItem::matStr() {
    ...
    for(int i=0; i<h; ++i) {
        QStringList line;
        for(int j=0; j<w; ++j) {
            line << QString::number(m_mat.at<float>(i, j));
        }
...
void CVImageItem::gaussianMat(cv::Mat_<float> const& srcMat) {
    std::cout << "gaussianMat" << std::endl;
    auto temp = srcMat;
    cv::GaussianBlur(temp, m_mat, cv::Size(3, 3), 0);
    update();
}
