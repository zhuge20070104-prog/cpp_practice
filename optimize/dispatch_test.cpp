...
public:
    ...
    int test_me_novirtual() {
        return test_me();
    }

    int test_me_virtual() override {
        return test_me();
    }
};
...
int test_me_switch(CDispatch* disp) {
    ...
    case ONE: 
        return ((CDispatchImpl<ONE>*)disp)->test_me_novirtual();
    case TWO:
        return ((CDispatchImpl<TWO>*)disp)->test_me_novirtual();
    case THREE:
        return ((CDispatchImpl<THREE>*)disp)->test_me_novirtual();
    case FOUR:
        return ((CDispatchImpl<FOUR>*)disp)->test_me_novirtual();
    }
}
...
void execute_1(std::vector<int>& results, std::vector<std::shared_ptr<CDispatch>> const& parameters) {
    clock_t begin = clock();
...
void execute_2(std::vector<int>& results, std::vector<std::shared_ptr<CDispatch>> const& parameters) {
    clock_t begin = clock();
    for(int b=0; b<ITERATIONS; ++b) {
        for(int a=0; a<SIZE; ++a) {
