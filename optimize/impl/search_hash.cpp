...
# include "stopwatch11.h"	//  stopwatch class
...
inline std::size_t hash_charbuf(charbuf<> const& cb) {
    return (size_t) cb;
}

// free function for hashing a char* string
inline std::size_t hash_charstar(char const* p) {
    std::size_t hash = 0;
    for(;*p; ++p) {
...
int test_hashes(int test_no, unsigned long multiplier) {
    ...
    case 2: {
        // hash<string>
        {
            ...
            StopWatch sw("unordered_map<string>");
        ...
        {
            std::unordered_map<charbuf<>, unsigned, std::hash<size_t>> table;
            for(auto it=std::begin(names); it!=std::end(names); ++it) {
                table[it->key] = it->value;
            }

            StopWatch sw("unordered_map<charbuf> operator size_t");
            for(unsigned long j=0; j<multiplier; ++j) {
                for(unsigned i=0; i<std::size(test); ++i) {
                    auto it = table.find(test[i].key);
                }
        ...
        // hash<charbuf<>, hash> function object
        {
            ...
            StopWatch sw("unordered_map<charbuf, hash> function object");
            for(unsigned long j=0; j<multiplier; ++j) {
                for(unsigned i=0; i<std::size(test); ++i) {
                    auto it = table.find(test[i].key);
                }
         ...
         // hash<charbuf, hash> lambda
        {
            ...
            StopWatch sw("unordered_map<charbuf, hash> lambda");
            for(unsigned long j=0; j<multiplier; ++j) {
                    for(unsigned i=0; i<std::size(test); ++i) {
                        auto it = table.find(test[i].key);
                    }
        ...
        // hash<charbuf, hash> free function
        {
            ...
            StopWatch sw("unordered_map<charbuf, hash> free function");
            for(unsigned long j=0; j<multiplier; ++j) {
                    for(unsigned i=0; i<std::size(test); ++i) {
                        auto it = table.find(test[i].key);
                    }
        ...
        // hash<char const*, hash> function object
        {
            ...
            StopWatch sw("unordered_map<char*, hash> function object");
            for(unsigned long j=0; j<multiplier; ++j) {
                    for(unsigned i=0; i<std::size(test); ++i) {
                        auto it = table.find(test[i].key);
                    }
        ...
        // hash<char const*, hash> lambda
        {
            ...
            std::unordered_map<char const*, unsigned, decltype(hash_lambda), comp_c_string> table(100, hash_lambda);
            for(auto it=std::begin(names); it!=std::end(names); ++it) {
                table[it->key] = it->value;
            }

            StopWatch sw("unordered_map<char*, hash> lambda");
            for(unsigned long j=0; j<multiplier; ++j) {
                    for(unsigned i=0; i<std::size(test); ++i) {
                        auto it = table.find(test[i].key);
                    }
        ...
        // hash<char const*, hash> free function
        {
            ...
            StopWatch sw("unordered_map<char*, hash> free function");
            for(unsigned long j=0; j<multiplier; ++j) {
                    for(unsigned i=0; i<std::size(test); ++i) {
                        auto it = table.find(test[i].key);
                    }
