...
template <typename T=int, std::size_t max=20>
class BoxContainer {
...
public:
    ...
    void operator+=(const BoxContainer<T, max>& other);
...
template <typename T, std::size_t max>
void BoxContainer<T, max>::operator+=(const BoxContainer<T, max>& other) {
...
template <typename T, std::size_t max>
std::ostream& operator<<(std::ostream& os, const BoxContainer<T, max>& box) {
