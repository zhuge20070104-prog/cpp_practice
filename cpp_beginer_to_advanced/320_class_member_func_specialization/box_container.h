...
T BoxContainer<T>::get_max() const {
    std::cout << "Non-specialized get_max() called" << std::endl;
    std::size_t max_index {};
    for(std::size_t i=0; i<m_size; ++i) {
        if(m_item[i] > m_item[max_index]) {
            max_index = i;
        }
    }
    return m_item[max_index];
}


template <> 
inline char* BoxContainer<char*>::get_max() const {
    std::cout << "Specialized get_max() called" << std::endl;
    std::size_t max_index {};
    for(std::size_t i=0; i<m_size; ++i) {
        if(std::strcmp(m_item[i], m_item[max_index]) > 0) {
            max_index = i;
        }
    }
    return m_item[max_index];
